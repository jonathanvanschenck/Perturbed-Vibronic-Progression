# -*- coding: utf-8 -*-
'''
Basically the same as `fit.py` except for fitting single VP PL signatures.
NOTE! Jnn/Ev must be set to frozen, becuase in absLineMu it is getting
overriden to 0.0, and that will totally jack up your fits.
'''
...
def absLineMu(x,_p,n,fun=Glorz):
    #p=area, Ex , Ev , S , sig0 ,Dsig , Jnn/Ev
    #   0    1    2    3     4    5       6
    p = _p*1
    p[6] = 0
    dx = np.absolute(np.mean(x[1:]-x[:-1]))
    x2 = np.arange(p[1]-(n+5)*p[2],p[1]+5*p[2],dx)
